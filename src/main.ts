...
import { Forecast, parseForecast } from './forecast.js';
...
const handleLocation = (msg: TelegramBot.Message): void => {
  const {
    chat: { id: chatId },
    from: { id: fromId },
...
const handleForecastRequest = async (
  ...
  const send = (forecast: Forecast): void => {
...
const sendFirstMessage = (msg: TelegramBot.Message): void => {
  const sorryText =
    'We are sorry, but button on PC/MAC does not work, you need to send location manually on MacOS or from your phone.';
